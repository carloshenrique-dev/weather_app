...
import 'package:weather_app/core/repositories/api_error.dart';
...
void main() {
  test('repository with mocked http client, success', () async {
    ...
    when(() => mockHttpClient.get(api.weather('Jundiai'))).thenAnswer(
        (_) => Future.value(http.Response(encodedWeatherJsonResponse, 200)));
    final weather = await weatherRepository.getWeather(city: 'Jundiai');
    // expectations
  ...
  test('repository with mocked http client, failure', () async {
    ...
    final weatherRepository =
        HttpWeatherRepository(api: api, client: mockHttpClient);
    when(() => mockHttpClient.get(api.weather('Jundiai')))
        .thenAnswer((_) => Future.value(http.Response('{}', 401)));
    // expectations
    expect(() => weatherRepository.getWeather(city: 'Jundiai'),
